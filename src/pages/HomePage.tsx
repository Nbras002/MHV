...
const HomePage: React.FC = () => {
  ...
  const { permits, addPermit, updatePermit, deletePermit, closePermit, reopenPermit, generatePermitNumber } = usePermits();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingPermit) {
      updatePermit(editingPermit.id, newPermit);
  ...
  const handleDelete = (permitId: string) => {
  ...
  const handleReopen = (permit: Permit) => {
    ...
    if (permit.closedAt) {
      ...
      if (hoursPassed > 1) {
        alert(t('permits.cannotReopenTime'));
        return;
      }
